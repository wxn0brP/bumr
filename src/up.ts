...
function convertVersionToSameLength(len: string, ver: string) {
	...
	const target = [];
	for (let i = 0; i < aLen; i++) target.push(splitB[i]);
	return target.join(".");
}
...
export async function upgradeDeps(opts: typeof options) {
	...
	const depsToUpdate = [];
	if (opts.dependencies && json.dependencies)
		depsToUpdate.push([
			"dependencies",
			json.dependencies,
		]);
	if (opts.devDependencies && json.devDependencies)
		depsToUpdate.push([
			"devDependencies",
			json.devDependencies,
		]);
	if (opts.peerDependencies && json.peerDependencies)
		depsToUpdate.push([
			"peerDependencies",
